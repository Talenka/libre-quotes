...
const GITHUB_ISSUES = 'https://github.com/Talenka/libre-quotes/issues';
...
$steps = array(array('text' => 'Check that you have PHP 5 or later, and MySql 4.1 or later'),
               array('text' => 'Check that PHP have write permission on <tt>core</tt> and <tt>cache</tt> directories'),
               array('text' => 'Create the <tt>core/config.php</tt> file'),
               array('text' => 'Create database tables with some data'),
               array('text' => 'Suppress the file <tt>install.php</tt> (Optionnal but recommended)'));

$messages = array();

if (PHP_VERSION >= 5 && class_exists('mysqli')) {
    $steps[0]['status'] = 'done';
    $step = 1;

} else $steps[0]['status'] = 'problem';
...
if ($step == 2 && file_exists('core/config.php')) {
    ...
    if ($dbObject->connect_error) {
        ...
        $steps[2]['status'] = 'problem';

    } else $steps[$step++]['status'] = 'done';
}
...
if ($step == 3) {
    ...
    elseif (!empty($_POST['create-tables'])) {
        ...
        $steps[3]['status'] = empty($messages) ? 'done' : 'problem';
    }
}
...
echo '<!doctype html>',
     ...
     '<style>',
     file_get_contents('views/style.css'),
     'label{display:block;margin:0 0 12px;font-size:80%;padding:0 3px}',
     'fieldset{border: 1px #4f443b solid;border-radius:10px}',
     '.done{color:#080}',
     '.current,.problem{font-weight:700}',
     '.problem{color:red}',
     '</style>',
     '</head>',
     '<body>',
     '<header><div><h1 id=title>', SITE_TITLE, '</h1></div></header>',
     '<main>',
     '<div>',
     '<nav>',
     'Report <a href="', GITHUB_ISSUES, '/new?labels=bug&amp;title=Problem+during+installation+process+step+', $step, '">bug</a>',
     '<a href="', GITHUB_ISSUES, '">suggestion</a>',
     '</nav>',
     '<h1>Hi fellow !</h1>',
...
foreach ($steps as $_step)
    echo '<li' . (empty($_step['status']) ? '' : ' class=' . $_step['status']) . '>' . $_step['text'] . '</li>';
...
<input type=text name=HOME id=HOME value="<?php echo HOME; ?>" maxlength=255 required>
<label for=HOME>Your site homepage URL relatively to this directory.</label>
<input type=text name=CACHE_PATH id=CACHE_PATH value="<?php echo CACHE_PATH; ?>" maxlength=255 required>
